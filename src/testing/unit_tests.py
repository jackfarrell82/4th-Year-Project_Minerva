...
## Load database connection config from local secure env file
...
## Class to test translation to SQL
class TestTextToSQL(unittest.TestCase):
    ## Testing the DB change function works
    ...
    ## Testing SQL translation for medical DB
    ...
    ## Testing SQL translation for financial DB
...
## Class to test Minervas pretrained responses
class TestMinervaPrompts(unittest.TestCase):
    ...
    ## testing different questions that can be asked
    ...
    ## details about whats in each DB
...
## helper function that takes in a prompt and sends it to minerva
def minerva(prompt):
...
## helper function to compare strings as they won't be character exact matches
...
## when file is run, start unittests
